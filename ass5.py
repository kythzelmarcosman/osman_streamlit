...
# Button for snapshot and stop video feed
...
while cap.isOpened():
    ...
    if snapshot:
        ...
        snapshot_image = frame_rgb  # Save image to display it in Streamlit
        st.success(f"Snapshot saved as {filename}")
        snapshot = False  # Reset snapshot button

    # Stop video feed if requested
    if stop_video:
        cap.release()
        st.warning("Video feed stopped.")
        break

# Display the snapshot image if available
